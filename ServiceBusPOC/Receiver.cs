namespace ServiceBusPOC
{
    ...
    public class Receiver
    {
        ...
        static async Task MainAsync()
        {
            ...
            await queueClient.CloseAsync();//.ConfigureAwait(true);
        }
        ...
        {
            //token = new CancellationToken(true);//nitish    

            // Process the message
            Console.WriteLine($"Received message: SequenceNumber:{message.SystemProperties.SequenceNumber} Body:{Encoding.UTF8.GetString(message.Body)}");
            //var msg = $"Received message: SequenceNumber:{message.SystemProperties.SequenceNumber} Body:{Encoding.UTF8.GetString(message.Body)}";
